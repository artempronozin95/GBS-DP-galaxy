...
rule all:
    input:
        expand("{ref}.dat.bwt", ref=config['reference_genome']),
...
rule cutadupt:
    ...
    run:
        for w in input:
            print(w)
            inp = w.rsplit('.', 1)
	    shell("mv {inp[0]}.dat {inp[0]}.fastq.gz")
            out = w.rsplit('/', 1)
            out = out[1].rsplit('.', 1)
            shell("cutadapt -a AGATCGGAAGAG {inp[0]}.fastq.gz  > triming_reads/{out[0]}_trim.fastq")

rule bwt:
   input:
           ref=ancient(expand("{ref}.dat", ref=config['reference_genome']))
   output:
           expand("{ref}.dat.bwt", ref=config['reference_genome'])
   run:
...
rule alignment:
    input:
         ...
         index=expand("{ref}.dat.bwt", ref=config['reference_genome']),
         ref=expand("{ref}.dat", ref=config['reference_genome'])
    params:
            thr=4
    output:
           expand("alignment/{sample}_trim.fastq.sam", sample=gunzip)
...
rule VCF:
    ...
    run:
       ...
       shell("VCF.sh {params.fold1} {params.fold2} {input.ref}")
...
rule index_vcf:
    ...
    output:
         "mod.txt"
    run:
         shell("index_vcf.sh {params.fold1} {params.fold2} {params.fold3} {params.fold4} {input.ref}")
